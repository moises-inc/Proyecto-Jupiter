...
Performs a full 14-zone multi-sector geographic risk scan across 100% of La Serena footprint,
calculating precise disaster types and Estimated Time of Arrival (ETA) for early warning.
"""
...
# 14 Geographic Sectors covering 100% of La Serena urban, rural, coastal and valley footprint
LA_SERENA_SECTOR_GRID = {
    "pueblo_islon": {
        ...
        "concentration_time_hours": 1.5, # Fast runoff from high mountain
    ...
    "las_companias_alta": {
        "name": "Las Compañías (Alta y Periurbano)",
    ...
    "compania_baja": {
        "name": "Compañía Baja & Sector Ribereño Norte",
...
def scan_full_la_serena_grid() -> dict:
    """
    Executes a 100% spatial grid scan across all 14 geographic sectors of La Serena.
    Computes precise disaster types and Estimated Time of Arrival (ETA).
    """
...
def print_full_scan_report(scan_data: dict):
    """
    Prints a clean, professional spatial scan report for La Serena without emojis.
    """
    print("\n" + "=" * 85)
    print(" PROYECTO CENTINELA — ESCANEO ESPACIAL COMPLETO DE LA SERENA (14 ZONAS)")
    print("=" * 85)
    ...
    print(f" {'#':<3} | {'Sector / Zona':<38} | {'Peligro Específico':<32} | {'ETA Impacto':<12} | {'Riesgo %':<8}")
    print("-" * 85)
    for idx, s in enumerate(scan_data["sectors"], 1):
        print(f" {idx:<3} | {s['name']:<38} | {s['disaster_type']:<32} | {s['eta_impact']:<12} | {s['score_pct']:<8.1f}%")
    print("=" * 85 + "\n")
